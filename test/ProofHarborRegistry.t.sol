...
contract ProofHarborRegistryTest is Test {
    ProofHarborRegistry private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_filecoin");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new ProofHarborRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        ProofHarborRegistry freshController = new ProofHarborRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
