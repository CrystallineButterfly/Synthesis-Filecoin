...
MODULE = importlib.import_module('agents.filecoin_proofs')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'filecoin_proofs')
        self.assertEqual(spec.primary_contract_name, 'ProofHarborRegistry')
